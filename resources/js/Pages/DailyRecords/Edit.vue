<template>
  <div>
    <Head :title="`Edit Daily Record - ${formatDate(dailyRecord.record_date)}`" />
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-3xl font-bold">
        <Link class="text-indigo-400 hover:text-indigo-600" href="/daily-records">Daily Records</Link>
        <span class="text-indigo-400 font-medium">/</span>
        {{ formatDate(dailyRecord.record_date) }}
      </h1>
    </div>
      ...
      <div class="mb-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex gap-2">
            <label for="file-upload" class="btn-kingbaker cursor-pointer">
              <input
                id="file-upload"
                type="file"
                class="hidden"
...
export default {
  ...
  data() {
    return {
      form: {
        record_date: this.dailyRecord.record_date,
        products: this.dailyRecord.products.map(product => ({
  ...
  methods: {
    ...
    submit() {
      this.$inertia.put(`/daily-records/${this.dailyRecord.id}`, this.form)
    },
  },
}
</script>